...
EXPECTED_WORKBOOKS = {
    ...
    "result3.xlsx": {"sheets": ["Sheet1"], "rows": 3486, "cols": 22, "first_time": 60, "last_time": 209100},
    "result4.xlsx": {"sheets": ["Sheet1"], "rows": 3194, "cols": 23, "first_time": 60, "last_time": 191580},
}

EXPECTED_DISTANCE_HEADERS = [round(i / 10, 1) for i in range(21)]
    ...
    try:
        ...
        for ws in wb.worksheets:
            ...
            check(
                ...
                f"{name}/{ws.title} A1 表头错误",
            )
            distance_headers = [ws.cell(1, col).value for col in range(2, 23)]
            check(distance_headers == EXPECTED_DISTANCE_HEADERS, f"{name}/{ws.title} 距离表头错误")
            if name == "result4.xlsx":
                check(ws.cell(1, 23).value == "药材表面", "result4.xlsx 缺少药材表面列")
