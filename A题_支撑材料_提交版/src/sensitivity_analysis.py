...
from . import physics, solver
from .config import ATTACHMENT_DIR, RADIUS_MAX_TIME_S
from .data_io import read_environment, read_radius
from .sampling import event_time, first_output_time_below
...
def one_case(parameter: str, factor: float) -> dict[str, float]:
    original_h = physics.HEAT_TRANSFER
    original_hm = physics.MASS_TRANSFER
    original_d = physics.DIFFUSIVITY_SCALE
    try:
        if parameter == "heat_transfer_h":
            physics.HEAT_TRANSFER = original_h * factor
        elif parameter == "mass_transfer_hm":
            physics.MASS_TRANSFER = original_hm * factor
        elif parameter == "diffusivity_scale":
            physics.DIFFUSIVITY_SCALE = original_d * factor
        elif parameter != "baseline":
            raise ValueError(parameter)

        environment = read_environment(ATTACHMENT_DIR / "附件1.xlsx")
        radius = read_radius(ATTACHMENT_DIR / "附件2.xlsx")
        q3 = solver.solve_fixed(
        ...
        event3 = event_time(q3)
        event4 = event_time(q4)
        safe3, _ = first_output_time_below(q3)
        safe4, _ = first_output_time_below(q4)
        return {
        ...
        physics.HEAT_TRANSFER = original_h
        physics.MASS_TRANSFER = original_hm
        physics.DIFFUSIVITY_SCALE = original_d
...
def run_sensitivity(print_output: bool = True) -> dict:
    ...
    output = {
        ...
        "baseline_parameters": {
            "heat_transfer_h_W_m2K": physics.HEAT_TRANSFER,
            "mass_transfer_hm_m_s": physics.MASS_TRANSFER,
            "diffusivity_scale": physics.DIFFUSIVITY_SCALE,
        },
        "baseline": baseline,
        "cases": cases,
    }
